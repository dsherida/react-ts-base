...
glob("assets/**/*", {}, (er, files) => {
    ...
    for (let i = 0; i < files.length; i++) {
        let file = files[i];
        if (!file.endsWith('.png') && !file.endsWith(".jpg") && !file.endsWith(".json") && !file.endsWith(".jepg")) continue;
        let piece = [file, ...file.replace('.png', '').replace('.jpeg', '').replace('.jpg', '').replace('.json', '').replace('assets/', '').split('/')];
        pieces.push(piece);
    }
